...
export default class BuilderStrategy extends RoomStrategyBase { 
    ...
    public run() : void {
        ...
        } else if (this.creepCount("courier") < 4 + this.energyCollectable/400) {
            this.trySpawnCreepFromBuildList(this.spawn, "courier", this.builds["courier"]);
        } else if (this.creepCount("builder") < this.constructionSites.length
                && this.creepCount("builder") < this.creepCount("upgrader")) {
